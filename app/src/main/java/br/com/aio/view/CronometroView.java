...
public class CronometroView extends View {
    ...
    public CronometroView(Context context) {
        super(context);
        init(context, null);
    }

    public CronometroView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public CronometroView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public CronometroView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CronometroView, 0, 0);
        try {
            mEmptyRingColor = ta.getColor(R.styleable.CronometroView_tickEmptyRingColor, mEmptyRingColor);
            mFillRingColor = ta.getColor(R.styleable.CronometroView_tickFillRingColor, mFillRingColor);
            mMiddleColor = ta.getColor(R.styleable.CronometroView_tickMiddleColor, mMiddleColor);
            mTextColor = ta.getColor(R.styleable.CronometroView_tickTextColor, mTextColor);

            mRingThickness = ta.getDimension(R.styleable.CronometroView_tickRingThickness, mRingThickness);
            mDotRadius = ta.getDimension(R.styleable.CronometroView_tickDotRadius, mDotRadius);
            mTextSize = ta.getDimension(R.styleable.CronometroView_tickTextSize, mTextSize);

            mText = ta.getText(R.styleable.CronometroView_tickText);
            mAutoFitText = ta.getBoolean(R.styleable.CronometroView_tickAutoFitText, mAutoFitText);

            mCounterClockwise = ta.getBoolean(R.styleable.CronometroView_tickMoveCounterClockwise, mCounterClockwise);
            mCircleDuration = ta.getInt(R.styleable.CronometroView_tickCircleDuration, mCircleDuration);
        } finally {
            ta.recycle();
        }
